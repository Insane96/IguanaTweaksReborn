...
public class SRBeaconRenderer implements BlockEntityRenderer<SRBeaconBlockEntity> {
    public static final ResourceLocation BEAM_LOCATION = new ResourceLocation("textures/entity/beacon_beam.png");
    public static final int MAX_RENDER_Y = 1024;

    public SRBeaconRenderer(BlockEntityRendererProvider.Context pContext) {
    }

    public void render(SRBeaconBlockEntity pBlockEntity, float pPartialTick, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight, int pPackedOverlay) {
        long i = pBlockEntity.getLevel().getGameTime();
        List<SRBeaconBlockEntity.BeaconBeamSection> list = pBlockEntity.getBeamSections();
        int j = 0;

        for(int k = 0; k < list.size(); ++k) {
            SRBeaconBlockEntity.BeaconBeamSection beaconblockentity$beaconbeamsection = list.get(k);

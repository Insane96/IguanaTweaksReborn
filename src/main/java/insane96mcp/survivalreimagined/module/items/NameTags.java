...
public class NameTags extends Feature {
    ...
    public void onRightClickEntity(PlayerInteractEvent.EntityInteract event) {
        ...
        if (target.getPersistentData().contains(SurvivalReimagined.RESOURCE_PREFIX + "has_name_tag"))
            dropNameTag(target.level(), target);
        target.getPersistentData().putBoolean(SurvivalReimagined.RESOURCE_PREFIX + "has_name_tag", true);
    }
    ...
    public void onEntityDeath(LivingDeathEvent event) {
        if (!this.isEnabled()
                || event.getEntity().level().isClientSide
                || !event.getEntity().getPersistentData().contains(SurvivalReimagined.RESOURCE_PREFIX + "has_name_tag"))
            return;

...
import insane96mcp.survivalreimagined.module.world.oregeneration.RandomBlockTagMatchTest;
...
public class SRRegistries {
    ...
    static <R> DeferredRegister<R> createRegistry(ResourceKey<? extends Registry<R>> key) {
        DeferredRegister<R> register = DeferredRegister.create(key, SurvivalReimagined.MOD_ID);
        REGISTRIES.add(register);
        return register;
    }

    static <R> DeferredRegister<R> createRegistry(IForgeRegistry<R> reg) {
        DeferredRegister<R> register = DeferredRegister.create(reg, SurvivalReimagined.MOD_ID);
        REGISTRIES.add(register);
        return register;
    }

    static <R> DeferredRegister<R> createRegistry(ResourceLocation registryName) {
        DeferredRegister<R> register = DeferredRegister.create(registryName, SurvivalReimagined.MOD_ID);
        REGISTRIES.add(register);
        return register;
    }

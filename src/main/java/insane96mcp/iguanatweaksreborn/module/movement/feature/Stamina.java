...
import insane96mcp.iguanatweaksreborn.event.PlayerSprintEvent;
...
public class Stamina extends Feature {
    ...
    public void onPlayerTick(TickEvent.PlayerTickEvent event) {
        ...
        if (player.isSprinting()) {
            ...
            shouldSync = true;
        } else if ((player.tickCount % 2 == 0 && getStamina(player) != getMaxStamina(player))
                || player.tickCount == 1) {
            regenStamina(player);
    ...
    public static void renderStamina(ForgeGui gui, PoseStack poseStack, float partialTicks, int screenWidth, int screenHeight) {
        ...
        RenderSystem.setShaderTexture(0, IguanaTweaksReborn.GUI_ICONS);
        int height = 9;
